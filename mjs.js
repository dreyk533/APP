...
function MatrixTable(sname, irows, icols, mtype, container) {
  ...
  this.insert = function (elements) { // mtype : 0 - single, 1 -  X, 2 - B, 3-Polynom
    ...
    st += tableHTML;
    st += '</div>';
    st += '<div class="nowrap"><label><input type="checkbox" class="swapMode" data-for="' + this.name + '" ' + (this.mode === 'cells' ? 'checked="checked"' : '') + ' /><ons-button class="slide-left effeckt-button topcoat-button no-select slide-left effeckt-button topcoat-button no-select topcoat-button-- swapMode">' + i18n.cells + '</ons-button></label>' +
          '&nbsp;'+
          '<ons-button class="slide-left effeckt-button topcoat-button no-select slide-left effeckt-button topcoat-button no-select topcoat-button-- clearTable" data-for="' + this.name + '">' + Utils.escapeHTML(i18n.clear) + '</ons-button>' +
          '&nbsp;'+
          '<ons-button class=" expression slide-left effeckt-button topcoat-button no-select slide-left effeckt-button topcoat-button no-select topcoat-button-- resizeTable" data-for="' + this.name + '" data-inc="+1">+</ons-button>' +
          '&nbsp;'+
          '<ons-button class=" expression slide-left effeckt-button topcoat-button no-select slide-left effeckt-button topcoat-button no-select topcoat-button-- resizeTable" data-for="' + this.name + '" data-inc="-1">&minus;</ons-button>' +
          '</div>';
...
function zInsAct(a, resultMatrix, loading, details) {
  ...
  var clearButton = '<ons-button  class="slide-left effeckt-button topcoat-button no-select slide-left effeckt-button topcoat-button no-select topcoat-button-- hidel" data-act-history-id="' + insertId + '">' + i18n.textClear + '</ons-button><br />';
  actHistory[insertId] = [a, resultMatrix, details]; // [String, String]

  var s = "";
  if (resultMatrix) {
    MatrixTables.forEach(function (table, id) {
      s += '<ons-button class="slide-left effeckt-button topcoat-button no-select slide-left effeckt-button topcoat-button no-select topcoat-button-- print-matrix" data-print-matrix-to="' + id + '">' + i18n.textInsertin + ' ' + id.toUpperCase() + '</ons-button><br />';
    });
  }
